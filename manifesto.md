...
The mechanism is dependency inversion applied to effects. The core does not call the database; it defines an interface (a Port) describing what it needs. The shell provides an implementation (an Adapter) satisfying that interface. The core remains pure; the shell remains logic-free; the boundary is explicit and enforced.
...
This separation enables powerful testing strategies. Domain logic can be tested by asserting on produced intents without any infrastructure. Service orchestration can be tested by providing fake port implementations that record calls. Integration tests verify only that real adapters correctly implement port interfaces. Each layer is testable in isolation because each layer's responsibilities are clearly bounded.
